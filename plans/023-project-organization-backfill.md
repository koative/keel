...
- [ ] **Step 1:** Understand the exact failure: a 0000-era DB has a `project` table with rows (columns per 0000: `id`, `name`, `slug`(?), `owner_id`, timestamps — read `0000_initial.sql` for the true 0000 project shape) and a `user` table, but **no** `organization`/`member` tables (0001 creates them). The repair must run AFTER 0001 has created organizations/members but BEFORE... no — the problem is 0001 itself fails on the non-empty project table. So the repair cannot run after 0001 if 0001 aborted.
