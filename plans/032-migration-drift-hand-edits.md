...
- [x] **Step 2:** Design the pass: for each table/column in the **current schema** (which the script can read via the drizzle schema module — check how the script imports `packages/db/src/schema` or whether it shells out), compare nullability against the latest snapshot that should contain it. The gap the hand-edit exposes: `created_by` is nullable in the schema and in the hand-edited 0001, but a *fresh* `drizzle-kit generate` would emit `ADD COLUMN ... NOT NULL`-era nullability differently across the rename — the snapshot's `created_by` entry is the tell. The precise assertion: **for every column in the latest migration snapshot, the schema's nullability must match**; where they differ, the script reports the column as drift (the hand-edit changed SQL without a schema change, or the schema changed without a migration — both directions report).
  ...
  - **Chosen design:** the pass replays what the committed migration *statements* declare about column nullability (CREATE TABLE columns, ADD COLUMN, ALTER COLUMN SET/DROP NOT NULL, RENAME COLUMN — line-oriented, drizzle-kit's emit format) and compares that against the schema's declared nullability, read offline via `packages/db/src/schema` (pure drizzle-orm; `@keel/db` itself is not importable — its index constructs a `pg.Pool` eagerly). The snapshot is deliberately *not* the comparison target: a snapshot serializes the schema's intent, not the SQL's effect — 0001's snapshot records `created_by` as nullable because the schema declared it so, while the generated rename would have kept `owner_id`'s NOT NULL and only the hand-added `DROP NOT NULL` makes the replay agree. Snapshot-vs-schema can therefore only see schema-ahead drift (which the probe already sees); the committed SQL is the only record of what the migrations actually do. The pass runs before the probe so it reports the *column* (the probe can only name the file it would have created); presence drift is left to the probe.
- [x] **Step 3:** Implement the chosen pass so that:
  ...
  - **Verified:** schema-ahead flip (createdBy → NOT NULL) reports `"project"."created_by": migrations declare nullable, schema declares NOT NULL`; migration-ahead hand-edit (temporary `SET NOT NULL` appended to 0006, reverted) reports `"project"."created_by": migrations declare NOT NULL, schema declares nullable` — the direction the generate probe cannot see; an extra column planted in the **schema** stays green for this pass and is caught by the probe naming `0008_drift-probe.sql` (probe intact). All planted changes reverted; final tree green: `check-migrations: 8 migration(s), schema matches.`
  - **Corrected after review:** the extra-column run above planted the column in the schema, which is the probe's case, not the migration-side hand edit this plan exists to catch. A column planted in a committed `.sql` left both passes green when this step was recorded. That gap is now closed: the replay reports a column the migrations declare and the schema lacks, verified by appending `ALTER TABLE "user" ADD COLUMN "is_admin" boolean NOT NULL;` to committed `0008` — `"user"."is_admin": the migrations declare this column, the schema does not` — plant reverted, tree green at 9 migrations. Still invisible to both passes, and stated as such in the script: a hand-edited DEFAULT or column type, a `CREATE UNIQUE INDEX` downgraded to `CREATE INDEX`, and a deleted `ADD CONSTRAINT`.
- [x] **Step 4:** Keep the output in the script's existing voice (`check-migrations: ...` summary line) and exit codes (0/1).
- [x] **Step 5:** Run `bun tools/check-migrations.ts` — green; run `bun run check`'s script chain — green (the lead runs the full gate).
- [x] **Step 6:** Commit: `feat(tools): migration drift now sees hand-edited SQL`.
