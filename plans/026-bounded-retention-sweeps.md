...
- **Error isolation in `tasks.ts`.** One failing sweep still aborts the run before the sweeps queued behind it. Batching removes the reason a sweep would fail on size, which is the failure this finding is about; making the script survive an arbitrary sweep failure — a `try`/`catch` per sweep and a non-zero exit that still reports what did run — is a separate change with its own acceptance test.
