...
- [ ] **Step 4:** Run the suite: `cd packages/storage && bun test src/client.test.ts` — green. If the first expected value is wrong (signature mismatch), that is the vector being wrong, not the code — re-derive the expectation independently before changing anything in the test's inputs.
- [ ] **Step 5:** Commit: `test(storage): pin the SigV4 signature to a known answer`.
