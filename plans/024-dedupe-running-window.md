...
- `job_dedupeKey_pending_idx` no longer exists in the schema or in any comment.
  > **Corrected.** This bullet also demanded it be gone "in the migrations", which is not achievable and should not be: `packages/db/src/migrations/0001_organizations_jobs.sql:95` created that index and `0005_dedupe_unsettled_window.sql:16` drops it. Migration history is append-only and `tools/check-migrations.ts` regenerates it, so the old name has to keep appearing in the two files that recorded its life. The schema and the comments are the part that can be clean, and are: a repository-wide grep for the old name outside `packages/db/src/migrations/` returns nothing.
- `bun run check` passes, reporting that every migration applies and the schema matches.
  > **Corrected.** This bullet used to pin the checker's line verbatim — `check-migrations: 5 migration(s), schema matches.` A running total is not this plan's to promise: `0006_idempotency_organization.sql`, `0007_webhook_event.sql` and `0008_sweep_indexes.sql` landed after it, and the count reads 9 at HEAD. What this plan is answerable for is its own migration and the absence of drift.
- No comment, README paragraph or AGENTS.md line still states that a claimed job frees its key.
  > **Was false when this plan closed; true now.** Two places still said it. `apps/server/src/lib/webhook.ts:181-188` named `job_dedupeKey_pending_idx` and its `WHERE status = 'pending'` predicate as the reason a `dedupeKey` cannot serve as a replay guard, sending a reader to a schema that had neither — corrected to `job_dedupeKey_unsettled_idx` and `status IN ('pending', 'running')`, with the paragraph's actual argument (the row is eventually swept, so the key is handed back) intact. `README.md`'s webhook paragraph said "the event id is what makes processing exactly-once, because `dedupe_key` is unique only while a job is pending", thirteen lines below a paragraph that correctly stated the two-status predicate — rewritten so the unique `(provider, event id)` row on `webhook_event` is named as the durable guard and the queue key only collapses retries still in flight.
